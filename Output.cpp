...
Output::Output()
{
	...
	UI.CellWidth = UI.width / NumHorizontalCells; // 1210/11=110
	UI.CellHeight = (UI.height -  UI.ToolBarHeight - UI.StatusBarHeight) / NumVerticalCells; // (610-55-60) /9 =55
...
void Output::DrawCardNumber(const CellPosition & cellPos, int cardNum) const
{
	...
	pWind->GetIntegerSize(w,h,cardNum); //returns the width and height of the integer drawn in the current font
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_ADD_COINSET] = "images\\Menu_CoinSet.jpg";
	MenuItemImages[ITM_ADD_CARD] = "images\\Menu_Card.jpg";	
	MenuItemImages[ITM_COPY_CARD_OR_COINS] = "images\\Menu_CopyCard.jpg";
	MenuItemImages[ITM_CUT_CARD_OR_COINS] = "images\\Menu_CutCard.jpg"; 
	MenuItemImages[ITM_PASTE_CARD_OR_COINS] = "images\\Menu_Paste.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	// First prepare List of images for each menu item
	// To control the order of these images in the menu, 
	// reoder them in UI_Info.h ==> enum PLAY_MODE_ITEMS
	// ** MAKE SURE THAT THE IMAGES ARE .JPG FILES **
	...
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\Menu_inputDice.jpg";
	MenuItemImages[ITM_OFFER_MORTGAGE] = "images\\Menu_OfferMortgage.jpg";
	MenuItemImages[ITM_ACCEPT_MORTGAGE] = "images\\Menu_AcceptMortgage.jpg";
	MenuItemImages[ITM_RETURN_MORTGAGE] = "images\\Menu_ReturnMortgage.jpg";
	MenuItemImages[ITM_SELLCITY] = "images\\Menu_SellCity.jpg";
	MenuItemImages[ITM_NEWGAME] = "images\\Menu_New.jpg";
...
void Output::DrawCell(const CellPosition & cellPos, int cardNum) const
{
	...
	int y = cellStartY + (UI.CellHeight - h - 1);  // space 1 from the end of the cell height
												   // ( - h ) because y is for the start point of cell num (num's upper corner)
...
void Output::DrawPlayer(const CellPosition & cellPos, int playerNum, color playerColor) const
{
	...
	if( playerNum < 0 || playerNum > (MaxPlayerCount-1) || cellPos.IsValidCell()==false)
		return;
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if(fromCell.VCell()<=toCell.VCell() || toCell.HCell()!=fromCell.HCell()) //(INVALID) because toCell is below, (INVALID) because NOT vertical
		return; //do NOT Draw 

	//toCell above fromCell means toCell(3,0) < fromCell(5,0)

	// Get the start X and Y coordinates of the upper left corner of the fromCell
	int cellStartX = GetCellStartX(fromCell); //i.e 0
	int fromStartY = GetCellStartY(fromCell); //i.e 330

	// Get the start Y coordinates of the upper left corner of the toCell (the X should be the same as fromCell .. Vertical)
	int toStartY = GetCellStartY(toCell); //i.e 220

	// ---- 1- Draw the First Vertical Line ---- 
	int x12 = cellStartX + UI.LadderXOffset; // the two points have the same x (vertical)  //i.e 19
	int y1 = fromStartY + UI.LadderYOffset;  // the coordinate y of the first point of the First Vertical line  //i.e 357
	int y2 = toStartY + UI.CellHeight - UI.LadderYOffset; // the coordinate y of the second point of the First Vertical line // i.e 248
						// (55-27)
	///TODO: Set pen color and width using the appropriate parameters of UI_Info object (UI)

	/*pWind->SetBrush(UI.LadderlineWidth);
	pWind->SetBrush(UI.LadderColor);
	pWind->SetPen(UI.LadderlineWidth);
	...
	int x34 = cellStartX + UI.CellWidth - UI.LadderXOffset; // same x (vertical line)
	                       //110-19=91                      // the y coordinates is the same as the First Vertical Line
	...
	//y1 = fromStartY + UI.LadderYOffset
	//y2 = toStartY + UI.CellHeight - UI.LadderYOffset
	for(int i=toStartY + UI.CellHeight; i<=fromStartY; i=i+UI.CellHeight)
	{
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if(fromCell.VCell()>=toCell.VCell() || toCell.HCell()!=fromCell.HCell() )
		return; //do NOT Draw

	//toCell below means greater than fromcell (to be drawn)
	...
	pWind->SetBrush(UI.SnakelineWidth);
	pWind->SetPen(UI.SnakeColor);
	pWind->SetPen(UI.SnakelineWidth);*/
	...
	int yUp=y1+yChange;  //(xLeft,yRight_left),(xRight,yRight_left),(x12,y1),(x12,yEnd)
	int yDown=y1-yChange;
	///TODO: Draw the Polygon (diamond) representing the Snake's Head
	//       Check the snakes drawn in the project document and draw it the same way
	
	pWind->DrawTriangle(xRight,y1,xLeft,y1,x12,yDown);
	pWind->DrawTriangle(xRight,y1,xLeft,y1,x12,yUp);

	//pWind->DrawTriangle(x12+xChange,y1,x12-xChange,y1,x12,y1-yChange);
	//pWind->DrawTriangle(x12+xChange,y1,x12-xChange,y1,x12,y1+yChange);

	//pWind->DrawTriangle(xLeft,yRight_left,x12,yEnd,xRight,yRight_left);
	//pWind->DrawTriangle(xLeft,yRight_left,x12,y1,xRight,yRight_left);

	// OR
	/*int x[]={x12,xRight,xLeft};
	int y[]={y1,yUp,yDown};
	pWind->DrawPolygon(x,y,4,FILLED);*/
}
//////////////////////////////////////////////////////////////////////////////////////////
void Output::DrawCoinSet(const CellPosition & cellPos) const  //NOTDONEEE----------------------------
{
	// Draws CoinSet image inside the cell of the passed cellPos
	// - CoinSet is drawn as an image
	// Don't forget any needed validations on cellPos
	if(cellPos.IsValidCell()==false)
		return;

	image CoinSetImage = "images\\Menu_CoinSet.jpg";
	int cellStartX = GetCellStartX(cellPos);
	int cellStartY = GetCellStartY(cellPos);
	...
	pWind->DrawImage(CoinSetImage,cellStartX,cellStartY,UI.MenuItemWidth, UI.ToolBarHeight); //55,55
}
//string MenuItemImages[ITM_ADD_COINSET];// = "images\\Menu_CoinSet.jpg";
//int x = cellPos.VCell();
//int y = cellPos.HCell(); these are wrong and will output image top left!
//int cellNum = cellPos.GetCellNum();	
// Draw menu item one image at a time
//for(int i=0; i < DESIGN_ITM_COUNT; i++)
	//pWind->DrawImage(MenuItemImages[i], i*UI.MenuItemWidth, 0, UI.MenuItemWidth, UI.ToolBarHeight);
